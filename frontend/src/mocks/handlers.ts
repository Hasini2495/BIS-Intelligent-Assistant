...
export const handlers = [
  http.post('/api/chat', async ({ request }) => {
    ...
    let answerObj;
    if (query.includes('concrete') || query.includes('is 456')) answerObj = chatResponsesFixture.concrete;
    else if (query.includes('water') || query.includes('drinking')) answerObj = chatResponsesFixture.water;
    else if (query.includes('hallmark') || query.includes('gold')) answerObj = chatResponsesFixture.hallmarking;
    else if (query.includes('certificat') || query.includes('isi')) answerObj = chatResponsesFixture.certification;
    else if (query.includes('earthquake') || query.includes('seismic')) answerObj = chatResponsesFixture.earthquake;
    else answerObj = chatResponsesFixture.default;
